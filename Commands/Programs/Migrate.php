...
//  - migrate(): void
//  - getClassNameFromMigrationFilename(string $filename): string
//  6
//  - getLastMigration(): ?string
//  - getAllMigrationFiles(string $order = 'asc'): array
//  - processQueries(array $queries): void
//  - insertMigration(string $filename): void
//  - rollback(int $n = 1): void
//  11
//  - removeMigration(string $filename): void
// =================
...
class Migrate extends AbstractCommand
{
  ...
  // ### マイグレーションの実行
  // まず、データベースから直近のマイグレーションを取得。その後、全てのマイグレーションファイルを日付順にソートして取得
  // そのファイルリストの中から、直近のマイグレーションの位置を見つけ出し、次のファイルをマイグレーションの起点として設定
  // 直近のマイグレーションに達するまで、全てのマイグレーションファイルのアップデート処理を順に実行
